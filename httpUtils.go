...
func HttpGet(dst_url string, proxy_str string, headerMap map[string]string, defaultCookieJar *cookiejar.Jar) (*http.Request, *http.Response, *http.Client, error) {
	client := GetClient(proxy_str, 0, defaultCookieJar, false)
	request, err := http.NewRequest("GET", dst_url, nil)
	if err != nil {
		log.Fatal(err)
		return request, nil, client, err
	}
	HttpHeaderBuild(request, headerMap)
	response, err := client.Do(request)
	if err != nil {
		log.Fatal(err)
		return request, response, client, err
	}
	return request, response, client, nil
}
